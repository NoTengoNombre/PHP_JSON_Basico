<div class="w6 STBBody Arial">
 ...
 <div id="tab3" class="contenido_tab w10 bordesGris" >
  <div  class="STBAgroup w10">
   <!--$info - Archivo JSON de tipo Array - String-->
   <?php
   ...
   <table id="tabla_documentos" class="STBTabla w10">
    ...
    <?php foreach ($info as $info) { ?>
        <tr class="row">
         ...
         <td class="boton" value="1"> 
          <button class="cambiar_estado">Marcar Impreso</button>
          </a>
         </td>
        </tr>
    <?php } ?>
...
<script>
    $(document).ready(function () {
        ...
        $(".cambiar_estado").click(function () {
            ...
            $(this).parent().parent().find(".status").html("Terminado");
            alert(id);
            $.ajax({
                url: "<?php echo site_url('controlador_documentos/cambiar_estado/') ?>" + id,
                success: function () {
                    $("#report").append("Estado modificado correctamente");


                },
                error: $("#report").append("Error al modificar"),
            });
        })
