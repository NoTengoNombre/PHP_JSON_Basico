	...
	public function check_login_modelo($usr, $pass) {
		...
		if ($this->db->affected_rows() > 0) { // si hay +1 fila
			...
			foreach ($resultado->result() as $fila) {//devuelve todos "elementos" en un array de objetos
				...
				$datos_usuario['idusr'] = $fila->usuario_id; // almaceno en el array $datos_usuario el objeto 'usuario_id' que es un String
				$datos_usuario['tipousr'] = $fila->tipo; // almaceno en el array $datos_usuario el objeto 'tipo' que es un String
			}
		} else {
